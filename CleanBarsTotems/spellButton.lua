if CleanBars.SpellButton then return end

local SpellButton = CleanBars:CreateClass('CheckButton', CleanBars.BindableButton)
CleanBars.SpellButton = SpellButton
...
function SpellButton:Create()
	local b = self:Bind(CreateFrame('CheckButton', string.format('CleanBarsSpellButton%d', self:GetNextID()), nil, 'SecureActionButtonTemplate, ActionButtonTemplate'))
...
function SpellButton:Skin()
	...
	if LBF then
		LBF:Group('CleanBars', 'Action Bar'):AddButton(self)
	else
