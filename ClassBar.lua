...
local ClassButton = CleanBars:CreateClass('CheckButton', CleanBars.BindableButton)
...
function ClassButton:Create(id)
    local name = format('CleanBarsClassButton%d', id)
    ...
    if ButtonFacade then
        ButtonFacade:Group('CleanBars', 'Class Bar'):AddButton(b)
    else
...
function ClassButton:OnEvent(event, arg1)
    if event == 'UPDATE_BINDINGS' then
        self:UpdateHotkey()
    elseif event == 'UPDATE_SHAPESHIFT_FORMS' and (self:GetID() > (GetNumShapeshiftForms() or 0)) then
        if not InCombatLockdown() then
            self:Hide()
        end
...
local ClassBar = CleanBars:CreateClass('Frame', CleanBars.Frame)
CleanBars.ClassBar  = ClassBar
