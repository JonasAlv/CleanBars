...
function Frame:GetDefaults()
    return {} -- Return an empty table so LoadSettings(the function right b) doesn't crash

	--[[idk why that happened, but it did. I guess the old code was relying on the fact that GetDefaults would return nil,
	and then LoadSettings would create a new table for sets. Now that GetDefaults returns an empty table, 
	LoadSettings will use that instead of creating a new one, which means that all frames will share the same sets table.
	This is not ideal, but it should work for now until we can figure out a better solution.]]--

end

function Frame:LoadSettings(defaults)
	self.sets = CleanBars:GetFrameSets(self.id) or CleanBars:SetFrameSets(self.id, self:GetDefaults()) 
	self:Reposition()
...
function Frame:ShowMenu()
	...
	if enabled then
		...
		if menu then
			menu:Hide()
			menu:SetOwner(self)
			menu:ShowPanel(CleanBarsConfigLocale.Layout)
			menu:Show()
		end
	end
end
