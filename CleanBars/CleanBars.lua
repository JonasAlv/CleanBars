...
function CleanBars:OnInitialize()
	...
	self.db = LibStub('AceDB-3.0'):New('CleanBarsDB', self:GetDefaults(), UnitClass('player'))
	...
	if LBF then
		LBF:RegisterSkinCallback('CleanBars', self.OnSkin, self)
	end
end
...
function CleanBars:ShowOptions()
	if LoadAddOn('CleanBarsConfig') then
...
function CleanBars:GetRightClickUnit()
...
function CleanBars:ShowBindingText()
...
function CleanBars:ShowMacroText()
...
function CleanBars:GetPossessBar()
	return self.Frame:Get(self.db.profile.possessBar)
end

function CleanBars:GetVehicleBar()
...
function CleanBars:SetNumButtons(count)
	self:SetNumBars(120 / count)
end

function CleanBars:NumBars()
	return self.db.profile.ab.count
end

function CleanBars:ShowTooltips()
	return self.db.profile.showTooltips
end

function CleanBars:SetShowTooltips(enable)
...
function CleanBars:ShowingMinimap()
...
function CleanBars:SetMinimapButtonPosition(angle)
	self.db.profile.minimapPos = angle
end

function CleanBars:GetMinimapButtonPosition(angle)
...
function CleanBars:Sticky()
	return self.db.profile.sticky
end

function CleanBars:SetLinkedOpacity(enable)
	self.db.profile.linkedOpacity = enable or false
end

function CleanBars:IsLinkedOpacityEnabled()
